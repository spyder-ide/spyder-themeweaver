...
Export also writes Spyder help `default.css`, `appeal.css`, and `pydoc.css` for each variant. Themes may override individual CSS color mappings in `mappings.yaml`; see [Help CSS overrides](#help-css-overrides), [Appeal CSS overrides](#appeal-css-overrides), and [Pydoc CSS overrides](#pydoc-css-overrides).
...
Optional `appeal_overrides` in a theme’s `mappings.yaml` replace entries from the built-in appeal CSS color map used for `appeal.css`. Omit the section to keep defaults. Keys are appeal CSS custom properties (for example `--link-color`). Values follow the same rules as `css_overrides`. Unknown keys are rejected.
...
Optional `pydoc_overrides` in a theme’s `mappings.yaml` replace entries from the built-in pydoc CSS color map used for `pydoc.css`. Omit the section to keep defaults. Keys are pydoc CSS custom properties (for example `--heading-color`). Values follow the same rules as `css_overrides`. Unknown keys are rejected.
